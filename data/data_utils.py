...
def perlin_noise(image, dtd_image, aug_prob=1.0):
    ...
    # i1 = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    # cv2.imwrite('./results_all2/' + 'ri2.png', img)
    # cv2.imwrite('./results_all2/' + 'ri1.png', i1)
    # cv2.imwrite('./results_all2/' + 'ri.png', (perlin_thr*255).astype(np.uint8))


    no_anomaly = torch.rand(1).numpy()[0] > aug_prob
