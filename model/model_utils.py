...
def l2_norm(input, dim=1, eps=1e-12):
...
def show_cam_on_image(img, anomaly_map):
    # print(img[0].shape)
    # print(anomaly_map[0].squeeze().shape)
    img = cv2.cvtColor(np.uint8(img[0].cpu()* 255), cv2.COLOR_BGR2RGB)
    anomaly_map = min_max_norm(anomaly_map)
    anomaly_map = cv2.applyColorMap(np.uint8(anomaly_map[0].squeeze().cpu()* 255), cv2.COLORMAP_JET)
    # print(img.shape)
